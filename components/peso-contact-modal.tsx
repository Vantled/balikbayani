...
export default function PesoContactModal({ open, onClose, initialData = null, onSuccess }: PesoContactModalProps) {
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        peso_office: initialData.peso_office,
        office_head: initialData.office_head
      })
      // For now, we'll use single email/contact from the database
      // In the future, we can extend the database to support multiple emails/contacts
      setEmails([initialData.email])
      setContacts([`Mobile No.: ${initialData.contact_number}`])
      setCustomCategories([""])
    } else {
      setFormData({
        province: "",
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.peso_office.trim()) {
      errors.peso_office = "PESO Office is required"
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // For now, we'll use the first email and contact for the database
      // In the future, we can extend the database to support multiple emails/contacts
      const pesoContactData = {
        province: formData.province.trim(),
        peso_office: formData.peso_office.trim(),
        office_head: formData.office_head.trim(),
        email: emails[0]?.trim() || "",
        contact_number: parsedContacts[0]?.contact_number || ""
      }

      const url = initialData ? `/api/peso-contacts/${initialData.id}` : '/api/peso-contacts'
      ...
      const response = await fetch(url, {
        ...
        credentials: 'include',
        body: JSON.stringify(pesoContactData),
      })

      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.error || `Failed to ${initialData ? 'update' : 'create'} PESO contact`)
      }

      await onSuccess?.()
      onClose()
    } catch (error) {
      console.error('Error saving PESO contact:', error)
        ...
        <div className="bg-[#1976D2] text-white px-6 py-4 flex items-center justify-between">
          <div className="flex items-center">
            ...
            <h2 className="text-lg font-medium">
              {initialData ? 'Edit PESO Contact' : 'Create PESO Contact'}
            </h2>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input 
                  ...
                  className={`mt-1 ${validationErrors.province ? 'border-red-500 focus:border-red-500' : ''}`}
                  placeholder="e.g., Cavite, Batangas"
                  required
                />
                {validationErrors.province && (
                  <p className="text-xs text-red-500 mt-1">{validationErrors.province}</p>
                )}
              </div>

              {/* PESO Office */}
              <div>
                <Label className="text-sm font-medium">PESO Office:</Label>
                <Input 
                  value={formData.peso_office}
                  onChange={(e) => {
                    setFormData({ ...formData, peso_office: e.target.value })
                    clearFieldError('peso_office')
                  }}
                  className={`mt-1 ${validationErrors.peso_office ? 'border-red-500 focus:border-red-500' : ''}`}
                  placeholder="e.g., Cavite PESO"
                  required
                />
                {validationErrors.peso_office && (
                  <p className="text-xs text-red-500 mt-1">{validationErrors.peso_office}</p>
                )}
              </div>
